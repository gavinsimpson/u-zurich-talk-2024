...
knitr::include_graphics("resources/bit.ly_uz-gam-talk.png")
```
...
```{r hadcrut-temp-example, echo = FALSE, out.width = "100%"}
## Load Data
URL <-  "https://bit.ly/hadcrutv4"
URL <- "https://bit.ly/hadcrutv5-annual"
gtemp <- read_csv(URL, col_types = "nnnn", col_names = TRUE) |>
  select(1:2) |>
...
```{r hadcrut-temp-polynomial, echo = FALSE, out.width = "100%"}
p <- c(1,3,8,15)
N <- 300
...
```{r hadcrtemp-plot-gam, echo = FALSE, out.width = "100%"}
library("mgcv")
...
ggplot(gtemp, aes(x = Year, y = Temperature)) +
  ...
  geom_line(data = pred,
            ...
            colour = "steelblue") +
  labs(x = "Year", y = expression(Temeprature ~ anomaly ~ degree*C))
```
...
```{r basis-functions, fig.height=6, fig.width = 1.777777*6, echo = FALSE, out.width = "100%"}
ggplot(basis,
...
```{r example-data-figure, fig.height=6, fig.width = 1.777777*6, echo = FALSE, out.width = "100%"}
...
```{r hadcrut-temp-penalty, echo = FALSE, out.width = "100%"}
K <- 40
...
# How has timing of peak incidence changed?
